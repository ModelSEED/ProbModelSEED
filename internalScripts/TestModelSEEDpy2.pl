...
if($parameters->{genome_type} eq "plant"){
	...
	for $item in @{$input->{media}->{mediacompounds}} {
		$item->{compound_ref} =~ s/idcpd/cpd/g;
	}
    ...
    my $cmd = Bio::KBase::utilities::conf("ProbModelSEED","internal_scripts_directory")."/ModelSEEDpy-ModelRecon.py ".$directory;
	#system($cmd);
	$data = Bio::KBase::ObjectAPI::utilities::FROMJSON(join(@{Bio::KBase::ObjectAPI::utilities::LOADFILE($directory."/output_model.json")},"\n"));
